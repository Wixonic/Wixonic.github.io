...
const RichLink = (url, el) => {
	...
	if (localEnvironment ? !url.hostname.endsWith("localhost") || url.host.endsWith("localhost:2014") : !url.hostname.endsWith("wixonic.fr") || url.hostname.endsWith("go.wixonic.fr")) {
		const data = {
			title: null,
			description: null,
			thumbnail: null,
